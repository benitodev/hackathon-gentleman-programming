...
const SearchSelectField = () => {
  ...
  useEffect(() => {
    ...
    if (episodesFiltered && episodesFiltered?.length !== lenghtConditional!)
      return
    ...
    const seasonsOptions = seasonsLabels.map((str, i) => ({
      label: str,
      value: seasonsValues[i],
    }))
    setOptions((prev) => ({
      ...prev,
      seasons: { inputLabel: 'Seasons', options: seasonsOptions },
    }))
  }, [data, episodesArray])
  ...
  useEffect(() => {
    ...
    const getEpisodesFromSeasonVar = episodesFiltered.filter(
      (str) => str.indexOf(filterEpisodes) !== -1
    )
    const getOnlyEpisodesData = sliceStringsInArray(
      getEpisodesFromSeasonVar,
      [3, 6]
    )
    const episodesLabels = sliceStringsInArray(getOnlyEpisodesData, [1])
    const episodesOptions = episodesLabels.map((str, i) => ({
      label: str,
      value: getOnlyEpisodesData[i],
    }))

    setOptions((prev) => ({
      ...prev,
      episodes: { inputLabel: 'Episodes', options: episodesOptions },
    }))
  ...
  return (
    <>
      {episodesFiltered && episodesFiltered.length >= lenghtConditional! && (
        <SelectField
          fields={{ seasons: options?.seasons, episodes: options?.episodes }}
        />
      )}
    </>
  )
