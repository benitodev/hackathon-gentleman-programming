...
type CharacterFromEpisodeProps = Pick<Character, 'id' | 'name' | 'species' | 'image' | 'gender'>
...
const CharacterCard = ({
  character, hasActionArea
}: CharacterCardProps) => {
  const {id, name, species, gender, image} = character
  return (
    <Card sx={{ maxWidth: 900, display: 'flex', flexDirection: 'row-reverse', width: '100%' }} >
    <Avatar
      alt={name as string}
      src={image as string}
    ...
    <CardContent sx={{display: 'flex', flexDirection:'column', width: '100%'}}>
      ...
      <Paper elevation={3} sx={{display:'inline', margin: '5px', padding: '5px'}} >
      <Typography component="h5" sx={{fontSize: '1rem', fontWeight:'500'}} color="text.primary">
        {species}
      </Typography>
      </Paper>
