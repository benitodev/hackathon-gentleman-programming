...
const EpisodeCard = ({ episode, cardActions = true, fontSizeTitle }: EpisodeCardProps) => {
  ...
  return (
    <Card sx={{ maxWidth: 345 }}>
      ...
      />
      <CardContent>
        <Typography gutterBottom variant="h3" color='primary.light' fontSize={fontSizeTitle}>
          {episode?.name}
        </Typography>
...

      {
        cardActions
          ...
          <Link to={`/characters/${episode?.id}`}>
            <CardActions>
              <Button size="medium" variant='outlined' color='secondary' fullWidth>Show characters</Button>
            </CardActions>
          </Link>
          :
          ''
